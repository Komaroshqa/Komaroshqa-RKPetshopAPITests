...
class TestPet:
    ...
    def test_getinformation_nonexistent_pet(self):
    ...
    def test_add_pet_full_data(self):
        ...
        with allure.step("Проверка параметров питомца в ответе"):
            ...
            assert response_json["photoUrls"] == payload["photoUrls"], "фото питомца не совпадают с ожидаемыми"
    ...
    def test_update_pet_information(self, create_pet):
        ...
        with allure.step("Отправка PUT-запроса на обновления информации о питомце"):
            response = requests.put(f"{BASE_URL}/pet", json=payload)
            update_json = response.json()
            assert response.status_code == 200, "Код ответа не совпал с ожидаемым"

        with allure.step("Проверка обновлённых параметров питомца в ответе"):
            assert update_json["id"] == payload["id"], "id питомца не совпадает с ожидаемым"
            assert update_json["name"] == payload["name"], "имя питомца не совпадает с ожидаемым"
            assert update_json["status"] == payload["status"], "статус питомца не совпадает с ожидаемым"
    ...
    def test_delete_pet_by_id(self, create_pet):
        ...
        with allure.step("Отправка GET-запроса на проверку удалённого питомца"):
            response = requests.get(f"{BASE_URL}/pet/{pet_id}")
            assert response.status_code == 404, "Код ответа не совпал с ожидаемым"
